...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <quaestoraddress> <amount>\n"
            "quaestoraddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <quaestoraddress> <amount>\n"
            "quaestoraddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1"
            + HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (strCommand != "listnodes" && strCommand != "start" && strCommand != "start-alias" && strCommand != "start-many" && strCommand != "start-all" && strCommand != "start-missing" &&
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in quaestor.conf\n"
                ...
                "  listnodes     - Print list of masternode over network\n"
                );
    ...
    if(strCommand=="listnodes"){
        ...
        BOOST_FOREACH(CMasternode& mn, vMasternodes) {
            ...
            obj.push_back(Pair("vin",           mn.vin.prevout.hash.ToString()));
            obj.push_back(Pair("pubkey",        CBitcoinAddress(mn.pubkey.GetID()).ToString()));
            obj.push_back(Pair("lastseen",      (mn.lastPing == CMasternodePing()) ? mn.sigTime :
                                                      (int64_t)mn.lastPing.sigTime));
            obj.push_back(Pair("activeseconds", (mn.lastPing == CMasternodePing()) ? 0 :

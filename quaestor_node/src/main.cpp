...
bool checkMnExist(CBitcoinAddress addressReceived)
{
    ...
    if (vMasternodes.size() > 0) {
       ...
       LogPrintf("checkMnExist() Node don't have Masternode list \n ");
        return true;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (currenBlockIndex->nHeight >= 166073) {
            if (tx.IsCoinBase()) {
    ...
    if (!IsBlockValueValid(block, GetBlockValue(pindex->pprev->nBits, pindex->pprev->nHeight, nFees),nFees,pindex->pprev->nHeight,block.vtx[0].GetValueOut())) {
